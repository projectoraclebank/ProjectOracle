                ...
                <?php
                  if(isset($_POST['login'])){
                    if(isset($_POST['loginUsername']) && isset($_POST['loginPassword'])){
                      ...
                      $requete="SELECT * FROM user INNER JOIN type_user ON type_user.idtype_user = user.id_type_user WHERE pseudo='$nomUS' AND password='$passUS' AND user.actif_user =1";
                      ...
                      if($count>0 && $count==1){
                        while($data_con=mysqli_fetch_assoc($result_connexion)){
                            ...
                            if($description_type_user != "ADMINISTRATEUR"){
                                session_destroy ();
                                $_SESSION[] = array();
                                ?>
...
//ALTER TABLE login_hystoric ADD FOREIGN KEY (user_login) REFERENCES user(iduser);
                                ...
                                $save_history = "INSERT INTO `login_hystoric` (`id_login`, `user_login`, `ip_login`, `date_login`) VALUES ('', '$id', '$ip',now())";
                                if(mysqli_query($connexion, $save_history)){
                                    if (mysqli_commit($connexion)){
                                        ...
                                        //erreur commit sauvegarde historique
                                        session_destroy ();
                                        $_SESSION[] = array();
                                        ?>
                                        <div class="alert alert-warning alert-dismissable">
                                            <button type="button" data-dismiss="alert" aria-hidden="true" class="close">&times;</button>
                                                <strong>Alerte!</strong> Erreur de connexion, essayer à nouveau.(2-3)
                                        </div>
                                        <?php  
                                    }
                                }else{
                                      //erreur sauvegarde historique de connexion
                                        session_destroy ();
                                        ...
                                        <div class="alert alert-warning alert-dismissable">
                                            <button type="button" data-dismiss="alert" aria-hidden="true" class="close">&times;</button>
                                                <strong>Alerte!</strong> Erreur de connexion, essayer à nouveau.(2-4)
                                        </div>
                                        <?php
                                }  
                            }
